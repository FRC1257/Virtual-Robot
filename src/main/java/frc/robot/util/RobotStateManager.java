...
public class RobotStateManager {
    ...
    public void scheduleMovement(TrajectoryManager manager) {
        updateStateFromPosition(manager.getStartPose(), checkIsBlue());
        if (currentState == States.PICKUP || currentState == States.SCORE) {
            return;
        }
        CommandScheduler.getInstance().schedule(
            manager.goToPos(getPoseFromState(checkIsBlue()))
        );
        resetNode();
    }

    public Command getMovement(TrajectoryManager manager) {
        updateStateFromPosition(manager.getStartPose(), checkIsBlue());
        if (currentState == States.PICKUP || currentState == States.SCORE) {
            return new InstantCommand();
        }
        return manager.goToPos(getPoseFromState(checkIsBlue())).andThen(new InstantCommand(() -> resetNode()));
    }
