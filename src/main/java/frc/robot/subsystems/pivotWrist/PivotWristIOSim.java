...
import static frc.robot.Constants.PivotWrist.PivotWristSimConstants;
...
public class PivotWristIOSim implements PivotWristIO {
  ...
  private double m_armSetpointDegrees = PivotWristSimConstants.kDefaultWristSetpointDegrees;

  // The arm gearbox represents a gearbox containing two Vex 775pro motors.
  private final DCMotor m_armGearbox = DCMotor.getVex775Pro(2);

  // Standard classes for controlling our arm
  private final ProfiledPIDController m_controller = new ProfiledPIDController(PIVOT_WRIST_PID[0], PIVOT_WRIST_PID[1], PIVOT_WRIST_PID[2],
  new TrapezoidProfile.Constraints(2.45, 2.45));
  private final Encoder m_encoder =
      new Encoder(PivotWristSimConstants.kEncoderAChannel, PivotWristSimConstants.kEncoderBChannel);
  private final PWMSparkMax motor = new PWMSparkMax(PivotWristSimConstants.kMotorPort);
  ...
  private SingleJointedArmSim sim = new SingleJointedArmSim(
        m_armGearbox,
        PivotWristSimConstants.kWristReduction,
        SingleJointedArmSim.estimateMOI(PivotWristSimConstants.kWristLength, PivotWristSimConstants.kWristMass),
        PivotWristSimConstants.kWristLength,
  ...
  private final EncoderSim m_encoderSim = new EncoderSim(m_encoder);
    ...
    public PivotWristIOSim() {
        m_encoderSim.setDistancePerPulse(PivotWristSimConstants.kWristEncoderDistPerPulse);
    }
